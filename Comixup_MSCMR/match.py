...
def to_onehot(idx, n_input, device='cuda'):
    ...
    idx_onehot = torch.zeros((idx.shape[0], n_input),
                             dtype=torch.float32,
                             device=device)
...
def mix_input(mask_onehot, input_sp, target_reweighted, sc=None):
    ...
    n_output, height, width, n_input = mask_onehot.shape
    n_class = target_reweighted.shape[1]
    ...
    target = torch.sum(mask_onehot_im.unsqueeze(2) * target_reweighted.unsqueeze(0),
                       dim=1)
...
def graphcut_wrapper(cost_penalty,
    ...
    if n_label == 2:
        ...
        mask_idx_np = graphcut_multi(unary,
                                     ...
                                     algorithm='swap')
        mask_idx_onehot = to_onehot(torch.tensor(mask_idx_np,
                                                 device=device,
                                                 dtype=torch.long),
                                    n_input + 1,
                                    device=device).reshape(
            height, width, n_input + 1)
        ...
        mask_onehot_i = to_onehot(torch.tensor(mask_idx_np,
                                               device=device,
                                               dtype=torch.long),
                                  n_input,
                                  device=device).reshape(
            height, width, n_input)
...
if __name__ == '__main__':
    ...
    def brute_force(cost_matrix, n_output, beta, gamma, idx_list=None):
        ...
        for k, idx in enumerate(idx_list):
            z = np.zeros((n_output * n_block ** 2,
                          n_input))  # one-hot representation of output
            for i in range(n_output * n_block ** 2):
                z[i, idx[i]] = 1

            mask_onehot = torch.tensor(z.reshape(n_output, n_block, n_block,
                                                 n_input),
                                       device=cost_matrix.device)
            loss = obj_fn(cost_matrix, mask_onehot, beta / n_block ** 2,
                          gamma / n_block ** 2)
    ...
    idx_list = list(
        itertools.product(*b))  # all possible combination of outputs
    print("start brute search: ", len(idx_list))

    for _ in range(100):
        input_sp = torch.from_numpy(
            np.random.normal(0, 1, size=(n_input, 3, 64, 64))).cuda().float()
        target_reweighted = torch.eye(n_input).cuda().float()

        cost_matrix = torch.from_numpy(
            np.random.normal(0, 1, size=(n_input, n_block,
        ...
        for i in range(n_output // n_part):
            # s = time.time()
            mask_onehot = get_onehot_matrix(
                cost_matrix[i * n_part:(i + 1) * n_part].detach(),
                A[i * n_part:(i + 1) * n_part, i * n_part:(i + 1) * n_part],
                n_part,
                beta=beta,
                gamma=gamma,
                eta=eta,
            ...
            obj = obj_fn(cost_matrix, mask_onehot, beta / n_block ** 2,
                         gamma / n_block ** 2)
            obj_list.append(obj)
            print("obj (our): {:.3f}".format(obj))

            mask_onehot_rand = torch.bernoulli(
                torch.zeros_like(mask_onehot) + 0.5)
            obj_rand = obj_fn(cost_matrix, mask_onehot_rand, beta / n_block ** 2,
                              gamma / n_block ** 2)
