...
def mixup_graph(input1, target_reweighted, grad1, indices, block_num=2, alpha=0.5, beta=0., gamma=0., eta=0.2,
    ...
    if transport:
        ...
        # input1
        plan = mask_transport(mask, unary1_torch, mask1_pool, eps=t_eps)
        input1 = transport_image(input1, plan, batch_size, t_block_num, t_size)

        # input2
        plan = mask_transport(1 - mask, unary2_torch, mask2_pool, eps=t_eps)
...
def mask_transport(mask, grad_pool, target_reweighted, eps=0.01):
