...
class SetCriterion(nn.Module):
    ...
    def Lv(self, outputs, targets):
        ...
        dice = (2 * torch.sum((src_masks == 3) * (targets_masks == 3), (1, 2)).float()) / (
                torch.sum(src_masks == 3, (1, 2)).float() + torch.sum(targets_masks == 3, (1, 2)).float() + 1e-10)
        return {"Lv": dice.mean()}
    ...
    def Myo(self, outputs, targets):
        ...
        src_masks = src_masks.argmax(1)
        targets_masks = targets.argmax(1)
        dice = (2 * torch.sum((src_masks == 2) * (targets_masks == 2), (1, 2)).float()) / (
                torch.sum(src_masks == 2, (1, 2)).float() + torch.sum(targets_masks == 2, (1, 2)).float() + 1e-10)
    ...
    def Rv(self, outputs, targets):
        ...
        src_masks = src_masks.argmax(1)
        targets_masks = targets.argmax(1)
        dice = (2 * torch.sum((src_masks == 1) * (targets_masks == 1), (1, 2)).float()) / (
                torch.sum(src_masks == 1, (1, 2)).float() + torch.sum(targets_masks == 1, (1, 2)).float() + 1e-10)
        return {"Rv": dice.mean()}
    ...
    def Avg(self, outputs, targets):
        ...
        for i in range(1, 4, 1):
            dice = (2 * torch.sum((src_masks == i) * (targets_masks == i), (1, 2)).float()) / (
    ...
    def loss_CrossEntropy(self, outputs, targets, eps=1e-12):
        ...
        flat_loss_weighted = 0.25 * (cross_entropy[:, 0, :, :] * 0.1 + (
                cross_entropy[:, 1, :, :] + cross_entropy[:, 2, :, :] + cross_entropy[:, 3, :, :]) * 0.3)
        losses = {
...
def build(args):
    ...
    weight_dict = {'loss_multiDice': args.multiDice_loss_coef,
                   'Rv': args.Rv,
                   'Lv': args.Lv,
                   'Myo': args.Myo,
                   'Avg': args.Avg,
